...
$(select).on('change', function () {
    document.getElementById("operation").innerHTML = select.value;

})

// var input2 = Number(myNumber.value)

$('#myNumber').on('change', function () {
    var input3 = Number(myNumber.value)

    document.getElementById("number_ope").innerHTML = input3;
...
interact('.heat').dropzone({
    accept: '#yes-drop2, #yes-drop4, #yes-drop5, #yes-drop6, #yes-drop8, #yes-drop1, #yes-drop3, #yes-drop7, #yes-drop9, #yes-drop10',
    overlap: 0.75,
    ...
    ondrop: function (event) {
        ...
        let clone1 = $(clone).clone();

        $(clone).css('visibility', 'hidden');

        console.log(Number($(clone1).text()));

        var input2 = Number(myNumber.value)


        var input = Number($(clone1).text())
        calc();
        Add();
        ...
        setTimeout(() => {
            $('#inner-dropzone1').html('');
            $('.drag-drop').removeClass('selected');
            $(clone).css('visibility', 'inherit');
        }, 4000);



        $(clone1).addClass('animate__animated animate__pulse lol animate_input_number');
        $('.animate_input_number').animate({
            left: '320px'
        }, 3000);


        // $(event.relatedTarget).addClass('selected');
        $('.drag-drop').addClass('selected');
        ...
        console.log(select.value + ` `+ input2);

        
        function Add() {
            var operation = select.value + ` `+ input2;
            calc();
            el = input;
            el2 = output;
            el3 = operation;
...
function addByIndex(index) {
    ...
    if (tasks.length <= 7) {
        setTimeout(() => {
            $('.field' + index).html(
                `<td>${tasks[index].one}</td>
                <td>${tasks[index].three}</td>
                <td>${tasks[index].two}</td>`
            );
        }, 3000);
    } else {
        setTimeout(() => {
            $('#tasks').append(` <tr class="field` + i + `">
            <td>${tasks[index].one}</td>
            <td>${tasks[index].three}</td>
            <td>${tasks[index].two}</td>
            </tr> 
...
function fnReloadAll() {
    ...
    stopAudio();
    document.getElementById("number_ope").innerHTML = 0;
...
function fnSetPlayer() {
    ...
    if (totalItems == 1) {
        $('.navigationControls, .nextBtn, .showAnsBtn, .reloadBtnScreen, .backBtn, .pageNumber').addClass('hide');
    }
